...
def configORGS(name, path): # name means if of org, path describe where is the namespace yaml to be created. 	
	...
	nfsPathNamespace = path.replace("transform/../", os.environ['NFS_DIRECTORY'] + ("" if os.environ['NFS_DIRECTORY'][-1] == "/" else "/"))
	
	render(namespaceTemplate, path + "/" + name + "-namespace.yaml", org = name,
	pvName = name + "-pv",
	nfsPath = nfsPathNamespace,
	nfsServer = os.environ['NFS_SERVER']
	)
	...
	if path.find("peer") != -1 :
		...
		nfsPathPeer = os.environ['NFS_DIRECTORY'] + ("" if os.environ['NFS_DIRECTORY'][-1] == "/" else "/") + "channel-artifacts"
